...
:install
if not exist "%~dp0..\.venv_DEV\Scripts\python.exe" (
    echo Creating virtual environment...
    call py -3.12 -m venv "%~dp0..\.venv_DEV"
    if errorlevel 1 (
...
echo Upgrading pip...
call "%~dp0..\.venv_DEV\Scripts\python.exe" -m pip install --upgrade pip setuptools wheel
if errorlevel 1 (
...
call "%~dp0..\.venv_DEV\Scripts\python.exe" -m pip install -e %~dp0..
if errorlevel 1 (
...
call "%~dp0..\.venv_DEV\Scripts\python.exe" -c "from pxr import Usd; print('USD is available')" 2>nul
if errorlevel 1 (
    if exist "%USD_ROOT%\bin\usdview.bat" (
        ...
        call "%~dp0..\.venv_DEV\Scripts\python.exe" -m pip install "usd-core>=25.11"
    )
)
...
:run
call "%~dp0..\.venv_DEV\Scripts\python.exe" -m dedaverse run
